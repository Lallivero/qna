...
from learning_baseline.context_rnn_debug import ContextRnnAgent
...
class BoWContextAgent(ContextRnnAgent):
    ...
    def ConstructGraph(self, l2Regularizer=0):
        ...
        # average
        stepAxis = 1
        qCode = tf.reduce_sum(qRnnInput, reduction_indices=stepAxis)
        qCode = qCode * self.invQSeqLen
        paCode = tf.reduce_sum(pcRnnInput, reduction_indices=stepAxis) - tf.reduce_sum(paRnnInput, reduction_indices=stepAxis)
        paCode = paCode * self.invPaSeqLen
        # pcCode = tf.reduce_sum(pcRnnInput, reduction_indices=stepAxis)
        naCode = tf.reduce_sum(ncRnnInput, reduction_indices=stepAxis) - tf.reduce_sum(naRnnInput, reduction_indices=stepAxis)
        naCode = naCode * self.invNaSeqLen
        # ncCode = tf.reduce_sum(ncRnnInput, reduction_indices=stepAxis)
        ...
        # DEBUG
        if DEBUG:
            naScores = tf.Print(naScores, (tf.reduce_sum(naScores),  tf.reduce_sum(naScores) + tf.reduce_sum(paScores), tf.shape(naScores), tf.shape(paScores) ), summarize=10, message="training naScore" )
            paScores = tf.Print(paScores, (tf.reduce_sum(paScores), paScores, tf.shape(paScores) ), summarize=60, message="t graph paScore")
    ...
    def ConstructEvalGraph(self):
        ...
        # average
        stepAxis = 1
        qCode = tf.reduce_sum(qRnnInputEval, reduction_indices=stepAxis)
        aCode = tf.reduce_sum(cRnnInputEval, reduction_indices=stepAxis) - tf.reduce_sum(aRnnInputEval, reduction_indices=stepAxis)
        qCode = qCode * self.invQSeqLenEval
        aCode = aCode * self.invASeqLenEval
        ...
        # DEBUG
        if DEBUG:
            self.evalScore = tf.Print(self.evalScore, (tf.reduce_sum(self.evalScore), tf.shape(self.evalScore) ), summarize=60, message="test sum score")
    ...
    def GetNextTrainBatch(self):
        ...
        denominator = batchData[self.pcSeqLen] - batchData[self.paSeqLen]
        denominator = np.where(denominator != 0, denominator, 1)
        
        # # DEBUG
        # print "pa denominator ", denominator


        batchData[self.invPaSeqLen] = 1 / np.tile(denominator, (self.nEmbedDim, 1) ).T.astype(floatType)
        denominator = batchData[self.ncSeqLen] - batchData[self.naSeqLen]
        denominator = np.where(denominator != 0, denominator, 1)


        # # DEBUG
        # print "na denominator ", denominator


        batchData[self.invNaSeqLen] = 1 / np.tile(denominator, (self.nEmbedDim, 1) ).T.astype(floatType)
        return batchData
    ...
    def GetPredictBatch(self, batchData):
        ...
        denominator = batchData[self.cSeqLenEval] - batchData[self.aSeqLenEval]
        denominator = np.where(denominator != 0, denominator, 1)
        batchData[self.invASeqLenEval] = 1 / np.tile(denominator, (self.nEmbedDim, 1) ).T.astype(floatType)
...
if __name__ == "__main__":
    ...
    if flags.FLAGS.doRestore:
        ckptFile = flags.FLAGS.ckptPath + "/model.ckpt-1001"
    ...
    agent = BoWContextAgent(nEmbedDim, nHidden, nLayer, cellType, 
        keepProb, floatType, idType, lossType, initStdDev, 
        negSampleSize, articleLevel, predTopK)
    ...
    # print "test length ", len(agent.trainSamples), len(agent.evalSamples)
    ...
    lrFunc = lambda iterNo: 3e-4 * 0.975**(iterNo/200) #if iterNo < 650 else 5e-3 * 0.975**(iterNo/200)
   
    # lrFunc = lambda iterNo: 0.0

    # best for no regularizer cases
    # lrFunc = lambda iterNo: 1e-2 * 0.975**(iterNo/200) if iterNo < 1000 else 5*1e-3 * 0.975**(iterNo/200)
    maxIter = 15000
