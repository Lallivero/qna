...
class QaData(object):
    def __init__(self, title, qaId, query=None, context=None, 
        ans=None, ansToken=None, paraId=None, senId=None, pAnsId=None, ansStr=None):
        '''
        ...
        if ansStr == None and self.ansToken != None and len(self.ansToken) != 0:
...
class Agent(object):
    ...
    def PrepareData(self, doTrain=True):
            ...
            for paragraph in article.paragraphs:
                for qa in paragraph.qas:
                    ...
                    if len(qa.answer.sentence) == 0:
                        ...
                        tokens = qa.answer.sentence[0].token
                        tokens = StandarizeToken(tokens)
                        if len(tokens) == 0:
                            continue
        ...
        #         candList = self.trainCandidates
        #     else:
        #         sampleList = self.evalSamples
        #         candList = self.evalCandidates
        #     for sample in sampleList:
        #         # if len(newSamples) >= self.batchSize + 1:
        #         #     break
        #         title = sample.title
        #         paraId = sample.pAnsParaId
        #         senIdProto = sample.pAnsSenId 
        #         for senId in range(len(candList[title][paraId] ) ):
        #             if senId != senIdProto:
        #                 continue
        #             if sample.ans in candList[title][paraId][senId]:
        ...
        #                 nCoverCnt += 1
        #                 break
        #     print "span coverage ", nCoverCnt, " / ", len(sampleList)
