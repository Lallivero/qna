...
DEFINE_int32(max_lex_dep_path_distance, 2, "");
...
FeatureDictionary::FeatureDictionary() {
  ...
  if (!FLAGS_input_dictionary.empty()) {
    ...
    read_only_ = false;
    output_ = OpenForWriting(FLAGS_output_dictionary);
  }
}
...
int FeatureDictionary::Count(const string& feature) {
  if (read_only_) {
    ...
    entry.set_name(feature);
    entry.set_index(num);
    CHECK(WriteDelimitedTo(entry, output_.get()));
...
void FeatureExtractor::ExtractFeatures(
  ...
  if (!FLAGS_only_numeric_features) {
    ...
    AddFeature(string("ParTreeLab = ") + parse_tree.value(), 1.0, features);
    if (wh_token_index != -1) {
      stringstream sout;
      sout << "Wh+ParTreeLab = " << wh_token_string << " " + parse_tree.value();
    ...
    AddFeature(string("AllPos =") + all_pos, 1.0, features);
    if (wh_token_index != -1) {
      stringstream sout;
      sout << "Wh+AllPos = " << wh_token_string << all_pos;
      AddFeature(sout.str(), 1.0, features);
    }
    ...
    AddLexicalizedOutOfSpanFeatures(sentence, preprocessed_sentence, question,
                                    span_begin_index, span_length, features);
    AddLexicalizedInSpanFeatures(sentence, question, span_begin_index,
                                 span_length, features);
  }
...
void FeatureExtractor::AddDepPathFeatures(
  ...
  for (const auto& question_lemma_and_pos :
    ...
    if (sentence_lemma_and_pos != preprocessed_sentence.lemma_pos.end()) {
      ...
      if (!ShortestPathToSpan(sentence, preprocessed_sentence, span_begin_index,
                              span_length, sentence_lemma_and_pos->second,
                              &distance_to_span, &closest_token_in_span)) {
        continue;
      }
      ...
      if (wh_token_index != -1) {
        ...
        if (distance_to_wh > FLAGS_max_dep_path_length) {
          ...
          wh_path = ConstructDepPath(
              question.sentence(0), preprocessed_question.preprocessed_sentence,
              wh_token_index, question_lemma_and_pos.second);
        }
      }
      ...
      for (bool include_wh_path : {false, true}) {
        ...
        sout << sentence.token(sentence_lemma_and_pos->second).pos()
             << answer_path << sentence.token(closest_token_in_span).pos();
...
void FeatureExtractor::AddLexicalizedOutOfSpanFeatures(
    const Sentence& sentence, const PreprocessedSentence& preprocessed_sentence,
    const edu::stanford::nlp::pipeline::Document& question,
    int span_begin_index, int span_length, CandidateAnswerFeatures* features) {
  for (int i = 0; i < sentence.token_size(); ++i) {
    if (i >= span_begin_index && i < span_begin_index + span_length) {
      continue;
    }
    int distance_to_span;
    int closest_token_in_span;
    if (!ShortestPathToSpan(sentence, preprocessed_sentence, span_begin_index,
                            span_length, i, &distance_to_span,
                            &closest_token_in_span) ||
        distance_to_span > FLAGS_max_lex_dep_path_distance) {
      continue;
    }

    const auto& sentence_token = sentence.token(i);

    for (int j = 0; j < question.sentence(0).token_size(); ++j) {
      const auto& question_token = question.sentence(0).token(j);

      stringstream sout;
      sout << "QL+SL+DPa = " << question_token.lemma() << " "
           << sentence_token.lemma()
           << ConstructDepPath(sentence, preprocessed_sentence, i,
                               closest_token_in_span);
      AddFeature(sout.str(), 1.0, features);
    }
  }
...
void FeatureExtractor::AddLexicalizedInSpanFeatures(
    const Sentence& sentence,
    const edu::stanford::nlp::pipeline::Document& question,
    int span_begin_index, int span_length, CandidateAnswerFeatures* features) {
  for (int i = span_begin_index; i < span_begin_index + span_length; ++i) {
    const auto& span_token = sentence.token(i);
    for (int j = 0; j < question.sentence(0).token_size(); ++j) {
      const auto& question_token = question.sentence(0).token(j);

      stringstream sout;
      sout << "QL+SpL = " << question_token.lemma() << " "
           << span_token.lemma();
      AddFeature(sout.str(), 1.0, features);
    }
  }
...
bool FeatureExtractor::ShortestPathToSpan(
    const Sentence& sentence, const PreprocessedSentence& preprocessed_sentence,
    int span_begin_index, int span_length, int from, int* distance_to_span,
    int* closest_token_index) {
  *distance_to_span = std::numeric_limits<int>::max();
  *closest_token_index = -1;
  for (int i = span_begin_index; i < span_begin_index + span_length; ++i) {
    if (uninteresting_pos_.count(sentence.token(i).pos())) {
      continue;
    }

    int shortest_dep_path = preprocessed_sentence.shortest_dep_path[from][i];
    if (shortest_dep_path < *distance_to_span) {
      *distance_to_span = shortest_dep_path;
      *closest_token_index = i;
    }
  }
  // If -1, only path goes through an uninteresting part of speech.
  return *closest_token_index != -1;
}
