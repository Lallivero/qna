...
class ContextScoreAgent(NonLearningAgent):
    ...
    def Predict(self, debug=False):
        ...
        #     self.predictions.update(self.PredictPerArticle(title) )
...
if __name__ == "__main__":
    ...
    dataFile = "./src/dev-annotated.proto"
   
    subAgent = SlidingWindowAgent(lambDist=0, lenPenalty=0.5, randSeed=0)
    subAgent.LoadStopWords()

    agent = ContextScoreAgent(1, 1, 0, randSeed=0, slidingWindowAgent=subAgent,
            articleLevel=False, topK=1)
    agent.LoadData(dataFile)
    agent.Predict(debug=False)
    # predFile = "./prediction.json"
    predFile = sys.argv[2]
...
#     # os.system("python evaluator.py dataset.json ./prediction.json")
