...
def GetQAPathPairs(origData, candData, anchorFunc=None):
    ...
    for qaId in ansSenDict.keys():
        if qaId not in aSpanDictKey \
            ...
            del ansSenDict[qaId]
            continue
        exactAns = Evaluator.CleanAnswer(' '.join([token.word.lower() for token in ansSenDict[qaId].token]))
        start = aSpanDict[qaId].spanBeginIndex
        end = start + aSpanDict[qaId].spanLength
        spanAns = Evaluator.CleanAnswer(' '.join([token.word.lower() for token in aSenDict[qaId].token[start:end] ]))
        if exactAns != spanAns:
            del qSenDict[qaId]
            del ansSenDict[qaId]
            del aSenDict[qaId]
...
if __name__ == "__main__":
    ...
    for dist in sorted(editDistGroup.keys() ):
        ...
        for qaId, _ in editDistGroup[dist]:
            ...
            if qaId in human_predictions:
                total_human_f1 += 100.0 * human_evaluator.F1Single(qaId, human_predictions[qaId])
                num_human += 1
        exactMatchRate = 0
        F1 = total_f1 / num_q
    ...
    # plt.hist(np.array(allDist), bins=20, range=(0, 20), normed=True, facecolor="green", alpha=0.75)
    ...
    plt.plot(np.array(sorted(editDistGroup.keys() ) ), np.array(F1List), \
        linewidth=2, label="Logistic Regression Dev F1")
    plt.plot(np.array(sorted(editDistGroup.keys() ) ), np.array(HumanF1List), \
        linewidth=2, label="Human Dev F1")
    plt.xlim( (0, 8) )
    plt.ylim( (20, 100) )
